...
import { _Notekle, _Seansekle, _Seansguncellemeislemi } from '../components/Firebaseislemleri';
...
const Seansguncelle=()=> {
    ...
    const[Seanstur,setseanstur]=useState(state.seanstur);
    const[Kilo,setkilodegisimi]=useState(state.kilo)
    const[Seansnot,setseansnot]=useState(state.seansnot);
    const[Seansadi,setseansadi]=useState(state.seansadi);

    const _Seansguncelle=()=>{
      
      if(Seanstur=="" || Kilo==""){
        
      }
      else{
            _Seansguncellemeislemi(state.key,state.donemisim,state.seanskey,Seansadi,Seanstur,Seansnot,Kilo,setprogress,navigate,state.danid)
  ...
  return (
    <div className='yeninot'>
      ...
      <h4 style={{fontSize:17,color:"#E7151F",marginBottom:0}}>{state.isim} {state.soyisim}</h4>
      <h4 style={{fontSize:17,color:"#333333",marginBottom:0,marginLeft:5}}>Kayıtlı Danışanın Dönem</h4>
      <h4 style={{fontSize:17,color:"#E7151F",marginBottom:0,marginLeft:5}}>{state.donemisim}</h4>
      <h4 style={{fontSize:17,color:"#333333",marginBottom:0,marginLeft:5}}>Kayıtlı Seans Bilgisi Güncelle</h4>
      ...
      style={{width:"100%"}}
        ...
        <Button onClick={()=>_Seansguncelle()} style={{textTransform: 'none',fontSize:15,color:"#333333",backgroundColor:"#B0BEC5"}} color="success" variant="contained" startIcon={<SaveIcon style={{fontSize:22}} />}>
