...
import { _ArsivDanisanlistele, _Danisanlistele, _Notlistele, _Notsil } from '../components/Firebaseislemleri';
...
import Mcikart from '../modal/Mcikart';
...
const Arsiv=()=>{
  ...
  useEffect(()=>{
      ...
      _ArsivDanisanlistele(setdanisanliste,Arama,setdogum,setkayit,setgenelprogess);
  ...
  return (
    <div className='notlarim'>
      ...
      <span style={{display:"flex",flexDirection:"row",alignItems:"center",justifyContent:"flex-start",backgroundColor:"#CFD8DC",padding:6,borderRadius:10,marginLeft:0}}>
        <LibraryBooksOutlinedIcon style={{fontSize:22,color:"#4CAF50",marginRight:5}} />
      {Danisanliste.length==undefined  ? <h4 style={{fontWeight:"bold",color:"#333333"}}>Arşivdeki Danışan Sayısı : 0</h4> : 
       <h4 style={{fontWeight:"bold",color:"#333333"}}>Arşivdeki Danışan Sayısı : {Danisanliste.length}</h4>  }
</span>
<IconButton onClick={()=>navigate("/Admin/Arsiv")}
     aria-label="delete">
        <NotificationsOffOutlinedIcon style={{color:"#FF0000",marginLeft:5}} />
           ...
           {searchParams.get("arsivcikart")=="basarili" &&  <Alert style={{width:500,marginBottom:10,fontSize:15,color:"#E8F5E9"}} variant="filled" severity="info">
        Danışan Arşivden Çıkartıldı !
      </Alert> }
      ...
      inputProps={{ style: {fontSize:15,color: '#333333',height:15,borderRadius:20 } }}
        ...
        <span style={{paddingRight:5,paddingLeft:5,padding:5,backgroundColor:"#CFD8DC",borderRadius:10,marginBottom:5,marginTop:10}}>
           <h4 style={{fontSize:16,color:"#333333",textAlign:"center",marginBottom:0}}>Arşivdeki Danışan Listesi</h4>
           </span>
      ...
      <CircularProgress style={{marginRight:0,marginLeft:10}} size={30}/>
           ...
           <div className='tabloarsiv-div'>
            <table className="tabloarsiv">
  ...
  <td>{item.meslek}</td>
    ...
    <td style={{width:150,display:"flex",flexDirection:"row"}}><IconButton onClick={()=>navigate("/Admin/Danisanlarim/"+item.danisanid,{state:{gorus:item.gorus}})}
     aria-label="delete">
      ...
      </IconButton><Msil danid={item.danisanid} id={"arsivsil"} baslik={item.isim+" "+item.soyisim} govde={"isimli danışanınızı silmek istediğinizden emin misiniz ?"} dankey={item.key}/>
     <Mcikart dankey={item.key} baslik={item.isim+" "+item.soyisim} govde={"isimli danışanınızı arşivden çıkartmak istediğinizden emin misiniz ? Evet derseniz danışanınız , Danışanlarım kısmını eklenecektir."} /></td> 
  </tr>
  )}
  </tbody>
  }
